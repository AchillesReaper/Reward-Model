...
from _vlmOffline import CriticOutput, MistralOffline
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from open_router_response_format import ResponseFormat
...
class Stage_1():
    def __init__(
        ...
        # --- construct model if run offline ---
        self.is_offline        = is_offline
        if self.is_offline:
            self.vlm_model = MistralOffline(model_id=model_id, max_new_tokens=max_tokens, torch_dtype=torch.bfloat16)
        ...
        self.output_file       = f'{cwd}/{trail_num}/sa_s1_{self.exp_task_name}.json'
    ...
    def single_sequence_describe(self, img_folder_path):
        ...
        if self.is_offline:
            ...
            critic_result = self.vlm_model.chat(messages)
